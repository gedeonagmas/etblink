...
const SalesDetail = ({ type }) => {
  ...
  return (
    <div
      className={`w-full ${
        type !== "company" ? "px-main pt-32" : "py-5 px-5"
      } z-20 h-[100vh]s flex flex-col items-center justify-center`}
    >
      <Response response={rateResponse} setPending={setPending} />
      {isFetching && <Loading />}
      {isError && type !== "company" && (
        <p>Something went wrong unable to read the data</p>
      )}
      {sales && location ? (
        <>
        ...
        type === "company" && <div>it seams you don't have sales yet</div>
      )}
      {/* ratings */}
    </div>
  );
